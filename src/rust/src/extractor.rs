...
async fn write_matching_reads<P>(fq: P, ofile: P, id_file: P) -> io::Result<()>
where
    P: AsRef<Path>,
{
    // Open input FASTQ
    ...
    let id_buf = std::io::BufReader::new(id_file);
    let id_set = id_buf
        .lines()
        .filter_map(|str| {
            str.ok().and_then(|str| {
...
fn extract_matching_sequence(
    ...
    rt.block_on(write_matching_reads(fq, ofile, id_file))
}
